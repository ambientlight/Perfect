...
public struct StaticFileHandler {
	...
	public func handleRequest(request: WebRequest, response: WebResponse) {
		...
		guard file.exists() else {
			...
			response.appendBodyString(string: "The file \(requestUri) was not found.")
	...
	func sendFile(response: WebResponse, file: File) {
		...
		do {
			...
			response.addHeader(name: "Content-type", value: MimeType.forExtension(ext: file.path().pathExtension))
			response.appendBodyBytes(bytes: bytes)
		} catch {
